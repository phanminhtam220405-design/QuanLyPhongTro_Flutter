...
class HouseListScreen extends StatelessWidget {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _showHouseForm(context, uid: uid),
          ),
        ],
      ),
        ...
        builder: (context, snapshot) {
          ...
          return ListView(
            ...
            children: snapshot.data!.docs
                .map((doc) => _buildHouseCard(context, doc))
                .toList(),
          );
        },
      ),
  ...
  Future<bool> _isAddressDuplicate(
    String address,
    String uid,
    String? currentDocId,
  ) async {
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () => _showHouseForm(
                          context,
                          uid: FirebaseAuth.instance.currentUser!.uid,
                          doc: doc,
                        ),
                        icon: const Icon(Icons.edit),
                        label: const Text("Sửa nhà"),
                        ...
                        onPressed: () => Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) => RoomListScreen(
                              houseId: doc.id,
                              houseName: data['address'],
                            ),
                          ),
                        ),
                        icon: const Icon(Icons.door_sliding),
                        label: const Text("DS Phòng"),
                        style: ElevatedButton.styleFrom(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              doc == null ? "Tạo thông tin căn nhà" : "Sửa thông tin căn nhà",
            ...
            TextField(
              controller: a,
              decoration: const InputDecoration(labelText: "Địa chỉ nhà *"),
            ),
            Row(
              children: [
                Expanded(
                  child: TextField(
                    controller: ep,
                    decoration: const InputDecoration(labelText: "Giá điện"),
                  ),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: TextField(
                    controller: wp,
                    decoration: const InputDecoration(labelText: "Giá nước"),
                  ),
                ),
              ],
            ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(u, textAlign: TextAlign.center)),
          Expanded(
            child: Text(
              p,
