...
class RoomListScreen extends StatelessWidget {
  ...
  // --- HÀM TẠO TÀI KHOẢN KHÁCH (DÙNG SECONDARY APP ĐỂ KHÔNG ĐĂNG XUẤT ADMIN) ---
  Future<String?> _createTenantAccount(String email, String name, String phone) async {
    try {
      ...
      // Lưu vào collection users để phân quyền login
  ...
  // --- GIAO DIỆN CARD PHÒNG (SỬA LẠI CHO GIỐNG CARD NHÀ) ---
  ...
  // --- FORM TẠO HỢP ĐỒNG (THÊM EMAIL VÀ TẠO ACC) ---
  void _showCreateContract(BuildContext context, DocumentSnapshot doc) {
    Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
    final tEmail = TextEditingController(); // Thêm Email
  ...
  void _showViewContract(BuildContext context, DocumentSnapshot doc) {
    Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => Container(height: MediaQuery.of(context).size.height * 0.85, decoration: const BoxDecoration(color: Color(0xFFF5F5F5), borderRadius: BorderRadius.vertical(top: Radius.circular(20))), padding: const EdgeInsets.all(20), child: SingleChildScrollView(child: Column(children: [const Text("Thông tin hợp đồng", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.green)), const SizedBox(height: 15), Row(children: [Expanded(child: _whiteInput("Phòng", data['name'] ?? '', null, enabled: false)), const SizedBox(width: 10), Expanded(child: _whiteInput("Số HĐ", data['contractNo'] ?? '', null, enabled: false))]), const SizedBox(height: 10), _whiteInput("Khách thuê", data['tenantName'] ?? '', null, enabled: false), const SizedBox(height: 10), Row(children: [Expanded(child: _whiteInput("Ngày ký", data['contractDate'] ?? '', null, enabled: false)), const SizedBox(width: 10), Expanded(child: _whiteInput("Người ở", "${data['occupantsCount']} người", null, enabled: false))]), const SizedBox(height: 10), _whiteInput("Tiền cọc", "${data['deposit']} đ", null, enabled: false), const SizedBox(height: 30), Row(children: [Expanded(child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red), onPressed: () { doc.reference.update({'isRented': false, 'tenantUid': FieldValue.delete()}); Navigator.pop(context); }, child: const Text("THANH LÝ", style: TextStyle(color: Colors.white)))), const SizedBox(width: 10), Expanded(child: ElevatedButton(onPressed: () => Navigator.pop(context), child: const Text("ĐÓNG")))])]))));
  }
